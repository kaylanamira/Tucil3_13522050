...
public class GBFS3 {
    ...
    public GBFS3(String startWord, String endWord, Dictionary dictionary) {
    ...
    public Word expandWord(){
        ...
        int min = selectedWord.getHeuristic();
        int idx = 0;
        for (int i = 1; i < size; i++){
            if (queue.get(i).getHeuristic() < min){
                selectedWord = queue.get(i);
                min = selectedWord.getHeuristic();
                idx = i;
            }
        }
        queue.remove(idx);
